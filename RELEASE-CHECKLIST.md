...
- [x] 已完成：用户从外部客户端验证 VLESS Reality、Hysteria2 直连与端口跳跃，以及订阅；Hy2 首次失败由云厂商入站防护拦截，放行 UDP 后通过。
...
- [ ] 不以 Reality 通过替代 Hy2 测试：Reality 仅验证 TCP 443，Hy2 必须单独验证 UDP 8443 与端口跳跃范围。
- [ ] 使用真实客户端确认端口跳跃范围可用，并核对订阅链接导入结果。
- [ ] 如需公开订阅，提供自管域名与 HTTPS 反向代理；默认令牌保护 HTTP 地址不应公开。

## 另一 agent 已完成的实测证据

- 已在全新 Debian 12 测试 VPS 完成两次前向部署：依赖安装、BBR、sing-box 校验、Reality/Hy2 服务、订阅服务、NAT 端口跳跃和 `postcheck` 均通过。
...
- [ ] 完成本轮修订的独立 Agent 前向测试。
- [x] 已完成外部客户端的 Reality/Hy2 真实握手测试。
